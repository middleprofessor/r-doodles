...
sigma <- 3
fake_data[, algae := (X%*%beta)[,1] + rnorm(n*2*2, sd=sigma)]

fit <- lm(algae ~ snail*crab, data=fake_data)
coef(summary(fit))
fit.emm <- summary(emmeans(fit, specs=c("snail", "crab")))

pd <- position_dodge(0.8)
gg_response <- ggstripchart(
  x = "snail",
  y = "algae",
  color = "crab",
  add = c("mean_ci"),
  ...
  #          position=pd, size=3) +
  geom_line(data=fit.emm, aes(x=snail, y=emmean, group=crab),
            position=pd) +
  NULL
