...
This is fake data that simulates an experiment to measure effect of treatment on fat weight. The treatment is "diet" with two levels: "control" (blue dots) and "treated" (gold dots). Diet has a large effect on total body weight. The simulated data look like that in the plot above (and very much like the real data).
...
A common assumption of ANCOVA is no treatment effect on the covariate. This doesn't make any sense, at least if the covariate is a mediator. Here is how I think about the generating model for these data -- or a series of generting models -- encoded by causal diagrams. The effect we're trying to estimate is the **direct** effect of diet on fat ($\beta_2$ in model 1, or $\beta$ in model 5). Model 1 is a typical "mediation" model. I don't care about the mediator, except as a nuissance. Model 2 is a bit more realistic in that body weight is a proxy for "size". Model 3 is thinking like Sewell Wright in his series of papers on "size factors" - which is **the set of foundational papers for causal modeling and DAGs**. Model 4 is just collapsing model 3 and model 5 is collapsing it a bit more (the special size factors aren't measured)
...
The issue that I see is not the effect of treatment on the covariate but that my covariate is a proxy for the latent factor. How does this bias my estimate of the direct effect of deit on fat? What is the best practice for this problem?
