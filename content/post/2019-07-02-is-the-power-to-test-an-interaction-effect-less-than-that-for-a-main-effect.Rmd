...
```{r echo=FALSE, eval=FALSE}
# compare expected mean given additive effecs using additive model vs. not including interaction effect
...
y0 <- (X0%*%beta0)[,1] + e
y1 <- (X1%*%beta1)[,1] + e
m0 <- lm(y0 ~ A + B, data=x_cat)
m1 <- lm(y1 ~ A*B, data=x_cat)
emmeans(m0, specs=c("A", "B"))
emmeans(m1, specs=c("A", "B"))
coef(m0)
coef(m1)
```
