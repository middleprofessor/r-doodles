...
A coefficient of a linear model is the difference in the modeled value on the link scale, given a one unit difference in the predictor,

\begin{equation}
b_j.k = (\hat{Y}|X_j=x+1, X_k=x_k) - (\hat{Y}|X_j=x, X_k=x_k)
\end{equation}

Because the modeled value of a linear model is a conditional mean, the coefficient is a difference in conditional means and therefore, is conditional on the additional covariates in the model ($X_k$).
...
1. Arguing that model selection and model averaging is mindless and careful selection of covariates is superior is not an argument against my rebuttal, only an argument against model selection and averaging more generally.

2. Arguing that model selection and model averaging is mindless and careful construction of a strucutural model based on prior knowledge is superior is not an argument against this rebuttal, only an argument against model selection and averaging more generally.

3. Arguing that my rebuttal is wrong because coefficients from non-linear models cannot be meaningfully averaged is not an argument against my rebuttal, only an argument on the limitation of model averaging coefficients (and again, contrary to [Cade 2015](https://scholar.google.com/scholar?hl=en&as_sdt=0%2C20&q=Model+averaging+and+muddled+multimodel+inferences&btnG=){target="_blank"} and [Dormann et al. 2018](https://scholar.google.com/scholar?hl=en&as_sdt=0%2C20&q=Model+averaging+in+ecology%3A+a+review+of+Bayesian%2C+information‐theoretic+and+tactical+approaches+for+predictive+inference&btnG=){target="_blank"}, coefficients of nested submodels from generalized linear models, which are *linear* models, can be meaningfully averaged).
