...
```{r simulation-gamma}
simulate_experiment_old <- function(
...
iterate_experiment_old <- function(
  n=10, # number of replicates per treatment level
  ...
  set.seed(1) # yes I want this here
  for(iter in 1:niter){
    fd <- simulate_experiment_old(
      n, # number of replicates per treatment level
      rho, # correlation between the reference value and that of a control level
      s_kappa, # effect of treatment on shape (this is multiplicative so 1 = no effect)
      s_theta, # effect of treatment on scale (this is multiplicative so 1 = no effect
...
```{r simulation2}
set.seed(1)
file_path <- here(output_path, "normalization_res2b.rds")
if(simulate_it==TRUE){
  res <- iterate_experiment_old(
  n=4, # number of replicates per treatment level
...
plot_effects(res)
# coef(summary(lm(effect_lm ~ delta_gapdh, data=res)))
plot_effects(resb[blots==1 & rho==0.5])
plot_effects(resb[blots==2 & rho==0.5])
plot_effects(resb[blots==3 & rho==0.5])
plot_effects(resb[blots==4 & rho==0.5])

```

The probability of Type I error as a function of observed difference in Gapdh value between treatment and control $\Delta Gapdh$. The nominal probability is 0.05. The conditional Type I error of the linear model gets slighlty conservative as the magnitude of $\Delta Gapdh$ increases. The conditional Type I error of norm1 (conventional normalization) gets slighlty liberal as the magnitude of $\Delta Gapdh$ increases. Compare this to no correlation with the reference value above. The conditional Type I error of norm2 (the normalization in the paper) is liberal at all magnitudes of $\Delta Gapdh$ increases..
...
Don't believe the higher power with norm2. It's a mirage due to the liberal Type I error.
...
### Conditional bias of effect estimate
#### one blot with all replicates, rho=0, effect=0

```{r}
plot_effects(res[blots==1 & rho==0])
```

#### 4 blots, each with a single replicate, rho=0, effect=0

```{r}
plot_effects(res[blots==4 & rho==0])

```

### Prob[type I] conditional on difference in Gapdh between Treatment and Control
#### rho=0
...
plot_prob_t1(res[blots==4 & rho==0])

```

```{r}
plot_prob_t1(res[blots==1 & rho==0.5])
plot_prob_t1(res[blots==2 & rho==0.5])
plot_prob_t1(res[blots==3 & rho==0.5])
plot_prob_t1(res[blots==4 & rho==0.5])
plot_prob_t1(res[rho==0.5])
