...
colnames(prob) <- c("cov", "norm1", "norm2")
effect_lm <- numeric(niter)
effect_norm1 <- numeric(niter)
effect_norm2 <- numeric(niter)
...
for(iter in 1:niter){
  ...
  effect_norm1[iter] <- mean(fd[treatment=="tr", norm1]) - mean(fd[treatment=="cn", norm1])
  effect_norm2[iter] <- mean(fd[treatment=="tr", norm2]) - 1
...
qplot(x=delta_gapdh, y=effect_norm1) + geom_smooth(method="lm")
qplot(x=delta_gapdh, y=effect_norm2) + geom_smooth(method="lm")
```
...
```{r simulation1-results}
prob_cols <- c("cov", "norm1", "norm2")
niter <- nrow(res)
apply(res[,.SD, .SDcols=prob_cols], 2, function(x) sum(x < 0.05)/niter)
qplot(x=res$delta_gapdh, y=res$effect_lm) + geom_smooth(method="lm")
qplot(x=res$delta_gapdh, y=res$effect_norm1) + geom_smooth(method="lm")
qplot(x=res$delta_gapdh, y=res$effect_norm2) + geom_smooth(method="lm")
