---
title: 'Normalization results in regression to the mean'
author: Jeff Walker
date: '2019-10-16'
...
tl;dr Standard procedure in labs is to normalize values from gels/blots/etc by some standard that is expected to be constant across treatments. A problem with this is *regression to the mean*, which results in inflated Type I error. This means t-tests and such are too liberal, that is, we reject the null to often, or we conclude "an effect exists" when the evidence doesn't favor this conclusion too often. The correct was to achieve the same goals as normalization is a linear model with the reference value as a covariate.
...
Intuition 1: Step one of this normalization would seem to introduce a biased estimate of the effect, conditional on Gapdh, very similar to the biased estimate of a change score (or change from pre to post) conditional on pre-treatment values [in a pre-post design](https://www.middleprofessor.com/files/quasipubs/change_scores.html).
...
plot_effects <- function(res){
  ...
  gg1 <- qplot(x=res$delta_gapdh, y=res$effect_lm) + geom_smooth(method="lm") +
    ggtitle("Linear model") +
    xlab(expression(paste(Gapdh[t] - Gapdh[c]))) +
    ylab("Effect")
  gg2 <- qplot(x=res$delta_gapdh, y=res$effect_norm1) + geom_smooth(method="lm") +
    ggtitle("Norm1") +
    xlab(expression(paste(Gapdh[t] - Gapdh[c]))) +
    ylab("Effect")
  gg3 <- qplot(x=res$delta_gapdh, y=res$effect_norm2) + geom_smooth(method="lm") +
    ggtitle("Norm2") +
    xlab(expression(paste(Gapdh[t] - Gapdh[c]))) +
    ylab("Effect")
...
res <- iterate_experiment(
  n=6, # number of replicates per treatment level
