...
```{r, fig.cap="p-values from linear model with Gapdh as covariate. The graphs are the same for all values of blots, but are shown for easy comparison with norm2 and norm3"}
plot_prob_t1_2(res, ycol="lm", two_d=TRUE)
```

```{r, fig.cap="p-values from t-test of norm1. The graphs are the same for all values of blots, but are shown for easy comparison with norm2 and norm3"}
plot_prob_t1_2(res, ycol="norm1", two_d=TRUE)
```

```{r, fig.cap="p-values from t-test of norm2"}
plot_prob_t1_2(res, ycol="norm2", two_d=TRUE)
```

```{r, fig.cap="p-values from t-test of norm3"}
